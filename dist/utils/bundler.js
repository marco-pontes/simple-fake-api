...
function loadUserConfigFile() {
    ...
    if (cfg) {
        if (DEBUG) {
            try {
                console.log('simple-fake-api/bundler[debug]: loadSimpleFakeApiConfigSync returned a config object');
            }
            catch { }
        }
        return cfg;
    ...
    if (DEBUG) {
        try {
            console.log('simple-fake-api/bundler[debug]: loadSimpleFakeApiConfigSync returned undefined; building tried paths list');
        }
        catch { }
    }
    const tried = [
    ...
    const msg = [
        ...
        'Config files are loaded via jiti at runtime; no need to install ts-node for configs. If your project uses ESM ("type":"module"), jiti handles interop (default export recommended).',
    ].join(' ');
    throw new Error(msg);
}
