...
vi.mock('path', async () => {
  const originalPath = await vi.importActual<typeof path>('path');
...
beforeEach(() => {
  ...
  (fs.readdirSync as Mock).mockReturnValue(['users.json']);
  (fs.readFileSync as Mock).mockReturnValue(mockUsersSchema);
  (generate as Mock).mockResolvedValue(mockGeneratedUsers);
});
...
describe('loadCollections', () => {
  ...
  it('should not load the collections again if they are already cached', async () => {
    ...
    (fs.readdirSync as Mock).mockClear();
    (fs.readFileSync as Mock).mockClear();
    (path.join as Mock).mockClear();
