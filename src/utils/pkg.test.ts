...
const makePkg = (overrides?: Record<string, any>) => {
  const base: any = { name: 'pkg-utils-test' };
  if (overrides) base['simple-fake-api-config'] = overrides;
  return JSON.stringify(base);
};

const makePkgWithHttp = (httpSection?: any) => {
  const base: any = { name: 'pkg-utils-test', 'simple-fake-api-config': {} };
  if (httpSection) base['simple-fake-api-config']['http'] = httpSection;
  return JSON.stringify(base);
};
